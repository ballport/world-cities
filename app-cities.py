...
st.title('World Major Cities - Ben Allport')

population_filter = st.slider('Choose Minimal Population', 0,40,15) #min, max, default

capital_filter = st.sidebar.multiselect('capital filter', ['primary', 'admin'], ['primary'])

#capital_filter = st.multiselect('capital filter', df.capital.unique(), df.capital.unique()) 
